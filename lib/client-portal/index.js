    ...
    io.on('connect', socket => {
        ...
        socket.on('joined-table', player => {
            if (this.activeGames.size < 1) {
                this.activeGames.set(gameid, poker.newGame('debug_poker', gameid));
            }

            const game = this.activeGames.get(gameid);

            const joinsuccess = (res) => console.log('join-success: ' + res);
            const joinerr = (err) => console.log('join-err: ' + err);
            const starthandler = () => console.log('start-handler: called for ' + socket.id);

            game.gameEvent.emit('player-joined', socket.id, joinsuccess, joinerr, starthandler, (seatIndex) => {
                console.log('player-joined-emit: pid ' + socket.id);
                console.log('player-joined-emit: seat pos index ' + seatIndex);
                if (seatIndex > 0) {
                    game.gameEvent.emit('player-seated', socket.id, () => {

                    });

                    socket.emit('player-seated', { seatIndex: seatIndex });
                }
            });
            ...
            //     //         socket.join(phases.predeal);

            //     //         if (gamestate === -1) {
            //     //             const playersInPredeal = socket.adapter.rooms[phases.predeal].length;
            //     //             if (playersInPredeal > 1) {
