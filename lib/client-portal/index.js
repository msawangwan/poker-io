...
class ClientPortal {
    ...
    setup() {
        ...
        if (!table) {
            table = new poker.Table(9, this.ws);
        }
    ...
    handleClientConnections() {
        ...
        io.on('connect', socket => {
            ...
            socket.on('get-turn-order', (data) => {
                const table = this.tables.get(data.tableid);

                table.tableEvent.emit('determine-turn-order', data.gameid, socket.id, (turnOrderIndex) => {
                    {
                        console.log('===');
                        console.log(`${socket.id} wants to know thier turn order ...`);
                        console.log('player turn order index is: ' + turnOrderIndex);
                        console.log('===');
                    }

                    socket.emit('turn-order-index', {
                        turnOrderIndex: turnOrderIndex
                    });
                });
            });

            socket.on('player-ready-up', (data) => {
                const table = this.tables.get(data.tableid);
                table.tableEvent.emit('player-ready-up', data.gameid, socket.id, () => {
