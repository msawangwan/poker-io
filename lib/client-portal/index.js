...
ClientPortal.prototype.handleClientConnections = function () {
    ...
    io.on('connect', socket => {
        ...
        socket.on('joined-table', player => {
            console.log('joined-table: player is joining table: ' + `tableid: ${debug.table.id} pid: ${socket.id}`)

            if (this.validTables.size < 1) {
                this.validTables.set(debug.table.id, new poker.Table(9));
            }

            const table = this.validTables.get(debug.table.id);
            ...
            const onsit = (seatIndex, gameId) => {
                ...
                socket.emit('player-seated', { seatIndex: seatIndex, gameId: gameId });
            };
            ...
            table.tableEvent.emit('player-request-seat', socket.id, onsit, onreject);
        });
