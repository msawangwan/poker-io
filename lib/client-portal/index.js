...
class ClientPortal {
    ...
    handleClientConnections() {
        ...
        io.on('connect', socket => {
            ...
            socket.on('get-dealer', (data) => {
                const table = this.tables.get(data.tableid);

                table.tableEvent.emit('determine-dealer', data.gameid, (db, sb, bb) => {
                    {
                        console.log('===');
                        console.log('sending dealer positions to all players');
                        console.log('db ' + db);
                        console.log('sb ' + sb);
                        console.log('bb ' + bb);
                        console.log('===');
                    ...
                    socket.emit('game-positions', {
                        button: db, smallblind: sb, bigblind: bb
                    });
                });
            });
