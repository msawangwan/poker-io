...
ClientPortal.prototype.handleClientConnections = function () {
    ...
    io.on('connect', socket => {
        ...
        socket.on('game-start', () => {
            ...
            const game = this.activeGames(gameinstanceid);
            
            game.gamestateNotifier.emit('game-started');
            
            this.activeGames.set(gameinstanceid, game);
            
            socket.emit('current-game-state', { state: game.state });
        });

        socket.on('ready-for-deal', () => {
            console.log('ready for deal: ' + socket.id);
