...
ClientPortal.prototype.handleClientConnections = function () {
    ...
    io.on('connect', socket => {
        ...
        socket.on('joined-table', player => {
            ...
            if (playerCount < 10) {
                console.log('new client connected while game is in state: ' + game.state);
                ...
                if (assignedSeatingPosition !== -1) {
                    ...
                    game.stateChangeEvent.emit('player-added', addedPlayer, assignedSeatingPosition, (gamestate) => {
                        // game.seating.set(assignedSeatingPosition, {
                        //     vacant: false,
                        //     player: {
                        //         name: `${player.name}`,
                        //         balance: player.balance,
                        ...
                        console.log(`number of players seated: ${game.seating.size}`);
                    ...
                    if (game.state === -1) {
                        ...
                        if (socket.adapter.rooms['waiting-room'].length > 1) {
                            game.enterNextState(game.state);
                        }
                    }
                    ...
                    // io.of('/').emit('current-game-state', { state: game.state }); // emits -1 orelse some other state everytime someonejoins
                }
            }
        });

        socket.on('player-joined-current-hand', () => {
