class Dealer {
    ...
    get players() {
        return [...this.seatIndexToPlayerMapping].filter(([k, v]) => v.id !== null);
    }
    ...
    get actingPlayer() {
        return [...this.seatIndexToPlayerMapping].find(([k, v]) => v.isActingPlayer);
    }
    ...
    toggleActingPlayer(previous, next) {
        if (previous !== null) {
            ...
            this.playerIdToSeatOrderMapping.get(p).isActingPlayer = false;
        }

        if (next !== null) {
            const n = this.turnOrderToSeatMapping.get(next);
            this.playerIdToSeatOrderMapping.get(n).isActingPlayer = true;
        }
    }
