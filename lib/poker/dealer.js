class Dealer {
    ...
    //     const prevState = this.findActivePlayerByBetOrder(curindex, 0, true);
    //     const nextState = this.findActivePlayerByBetOrder(curindex, 0, false);
    ...
    setPlayerStatesOnInit(positions, buttonPosition) {
        ...
        // for (const [i, s] of this.allValidPlayerStates) {
        //     if (s.isValid) {
        //         this.linkPlayerStateNodes(s);
        //     }
        // }
    ...
    setPlayerStatesOnRoundChange() {
        ...
        for (const [i, s] of states) {
            ...
            // const prev = s.player.previous;
            // const next = s.player.next;

            // if (!prev || !next) {
            //     this.linkPlayerStateNodes(s)
            // } else if (!prev.isValid || !next.isValid) {
            //     this.linkPlayerStateNodes(s)
            // }
        }
    }
    ...
    validatePlayerBet(playerState, completedAction, clientRound, betAmount) {
    ...
    validateRoundState(roundName, playerState, playerAction) {
        ...
        switch (this.activeRound.name) {
            ...
            case 'deal':
                if (playerState) {
                    if (playerState.position.isBigBlind && playerState.hasActedOnce) {
    ...
    verifyAllPlayersInHandOweZero() {
        ...
        for (const s of states) {
            if (s.hasFolded || s.actions.owed(this.minBet, s.actions.bets.total) === 0) {
                continue;
            }
