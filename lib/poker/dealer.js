class Dealer {
    ...
    get smallBlind() {
        return [...this.seatIndexToPositionStateMapping].find(([k, v]) => v.isSmallBlind)[1];
    }

    get bigBlind() {
        return [...this.seatIndexToPositionStateMapping].find(([k, v]) => v.isBigBlind)[1];
    }

    get dealerButton() {
        return [...this.seatIndexToPositionStateMapping].find(([k, v]) => v.isDealer)[1];
    }

    get actingPlayer() {
        return [...this.seatIndexToPositionStateMapping].find(([k, v]) => v.isActingPlayer)[1];
    }
    getNextPlayerByPrevious(playerState) {
        return this.seatIndexToPositionStateMapping.get(
            this.turnOrderToSeatMapping.get(
