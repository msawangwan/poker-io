class Dealer {
    constructor() {
        ...
        for (let i = 0; i < 9; i++) {
            ...
            // actions: {
            //     // betTotal: 0,
            //     // amountOwed: (min, total) => Math.abs(min - total),
            //     // lastActionId: (all) => all.length ? all[all.length - 1] : -1
            //     // historyIds: [],
            //     // lastActionId() { return this.historyIds.length ? this.historyIds[this.historyIds.length - 1] : -1 }
            //     bets: new Map([[-1, { id: -1, amount: 0 }]]),
            //     allowed: [],
            //     betSum() { return [...this.bets].reduce((b, s) => b[1].amount + s); },
            //     amountOwed(target) { return this.betSum() - target; },
            //     mostRecentActionId() {
            //         let hi = -1;

            //         for (const [i, b] of this.bets) {
            //             if (i > hi) {
            //                 hi = i;
            //             }
            //         }
        ...
        for (let i = 0; i < this.roundNameMap.length; i++) {
            this.rounds.set(i, {
                ...
                actions: {
                    bets: {
                        current: new Map([[-1, { id: -1, amount: 0 }]]),
                        previous: new Map([[-1, { id: -1, amount: 0 }]]),
                        sumCurrent(prev) {
                            if (prev) {
                                return [...this.previous].reduce((b, s) => s[1].amount + s);
                            }
                            return [...this.current].reduce((b, s) => s[1].amount + s);
                        },
                        total() {
                            return this.sumCurrent(false) + this.sumCurrent(true);
                        }
                        // thisRound: 0,
                        // lastRound: 0,
                        // total: (tr, lr) => tr + lr
                        // total() { return this.thisRound + this.lastRound; }
                    },
                    count: 0
                }
            });
        ...
        this.pot = {
            ...
            lastActionId: -1,
            history: new Map(),
            newActionId() { return this.lastActionId + 1; },
            lastActionCompleted() { return this.history.get(this.lastActionId); }
        };
    ...
    get haveAllPlayersActedOnce() {
        return [...this.playerStateTableBySeatIndex].every(([k, v]) => v.player.hasActedOnce);
    }
    ...
    // potHistoryActionByPlayerState(playerState) {
    // return this.pot.history.get(playerState.actions.lastActionId());
    // playerState.actions.lastActionId(playerState.actions.historyIds)
    // }

    // getLastActionCompleted() {
    //     return this.pot.lastActionId > 0 ? this.pot.history.get(this.pot.lastActionId) : null; // todo: dangerous
    // }
    ...
    processPlayerAction(playerState, completedAction, clientRound, betAmount) {
        ...
        const actid = this.pot.newActionId()
    ...
    //     // const states = this.allValidPlayerStates;
    //     const states = this.allAlivePlayerStates;

    //     console.log("VERIFY ALL PLAYERS OWE 0");

    //     for (const [i, s] of states) {
    //         // const owes = s.actions.amountOwed(this.minBet, s.actions.betTotal);

    //         console.log(`PLAYER ${s.player.id} OWES ${owes}`);

    //         if (s.player.hasFolded || owes === 0) {
    //             continue;
    //         }
