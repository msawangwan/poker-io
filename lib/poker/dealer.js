class Dealer {
    ...
    handleBlind(pid) {
        ...
        return this.handleBet(pid, bet, 'postblind', 'blind');
    }
    ...
    handleBet(pid, betAmount, action, phase) {
        ...
        current.bet.historyIds.push(this.actionId);
            ...
            if (action === 'fold') {
                ...
                if (action !== 'check') {
                    ...
                    if (betAmount > this.minBet) {
                        ...
                        current.isBetAnchor = true;

                        this.betAnchor.state = current;
                        this.betAnchor.actionId = this.actionId;
                    }
    ...
    handleBetAnchor(action) {
        if (action !== 'check') {
            // checked so we can go to next round
        } else {
            // otherwise figure out what we need to do
        }
    }
