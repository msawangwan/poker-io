...
class Dealer {
    constructor() {
        this.dealerStates = new Map([
            [0, new states.StartState(0, 'start', this)],
            [1, new states.BlindState(1, 'blind', this)],
        ]);
    ...
    handlePlayerRequest(pid, betAmount, clientAction, clientRound) {
        if (this.activePlayer.state.id !== pid) {
            Dealer.logActivePlayerSyncCorruption(this.activePlayer.state.id, pid);
        }

        this.stateHistoryIds.push(this.currentState.id);

        const playerAction = Dealer.createBet(
            0,
            pid,
            clientAction,
        ...
        const state = this.currentState.process(this.activePlayer, clientAction);
    }
    ...
    static createRoundTransaction(bet) {
        return {
