class Dealer {
    ...
    // TODO: Left of here !!!
    updateRoundStateOnPlayerCompletedAction(roundName, playerState, playerAction) {
        ...
        switch (this.activeRound.name) {
            ...
            case 'deal':
                if (playerState) {
                    if (playerState.position.isBigBlind && playerState.hasActed) {
    ...
    verifyAllPlayersInHandOweZero() {
        ...
        for (const p of players) {
            if (p.hasFolded || p.actions.owed(this.minBet, p.actions.bets.total) === 0) {
                continue;
            }
