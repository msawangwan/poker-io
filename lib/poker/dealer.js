class Dealer {
    ...
    // get lastActionId() {
    // return this.actionHistory ? this.actionHistory[this.actionHistory.length - 1].lastActionId : -1;
    // }
    set activeRound(r) {
    ...
    findActivePlayerByBetOrder(orderIndex, playersCounted, findBackwards) {
        ...
        // const i = findBackwards ? this.calcPrevIndexFrom(orderIndex) : this.calcNextIndexFrom(orderIndex);
    ...
    initialise(blindsize) {
        if (!this.dealerInit) {
            ...
            // this.rounds.get(1).isCurrentRound = true;
    ...
    validateSentStateMatchesServerState(playerid, clientRound) {
    ...
    processPlayerAction(playerState, completedAction, clientRound, betAmount) {
        ...
        // this.actionHistory.push({ lastActionId: actid, actionAmount: betAmount, actionOwner: playerState.player.id });
    ...
    // validateRoundState(roundName, playerState, playerAction) {
    //     let update = false;
    ...
    //     if (update) {
    //         // const cur = this.getRoundStateByName(roundName);
    //         const cur = this.activeRound;
    //         const curcomplete = this.markRoundComplete(cur);

    //         // const nxt = this.getNextRoundStateFromCurrentRoundState(curcomplete);
    //         const nxtactive = this.markRoundActive(nxt);

    //         const bets = curcomplete.actions.bets;

    //         nxtactive.actions.bets.lastRound = curcomplete.actions.bets.total(bets.thisRound, bets.lastRound);

    //         this.minBet = this.blindBetSize;
    //     }
    ...
    verifyAllPlayersInHandOweZero() {
        ...
        for (const [i, s] of states) {
            if (s.hasFolded || s.actions.owed(this.minBet, s.actions.betTotal) === 0) {
                continue;
            }
    ...
    getAllowedActionsForPlayer(state, roundName) {
        ...
        if ((state.position.isBigBlind || state.position.isSmallBlind) && !state.hasActedOnce) {
            return ['postblind'];
        }
