...
const dealerStates = require('./states');

class Dealer {
    constructor(options) {
        this.gameParameters = {
            maxBettingRoundsPerRound: options.maxBettingRoundsPerRound || 3
        };

        this.states = new Map([
            [0, new dealerStates.StartState(0, 'start')],
            [1, new dealerStates.BlindState(1, 'blind')],
        ]);
    }
    ...
    initialise() {
        ...
        this.states.get(0).isActive = true;

        while (i < this.states.size - 1) {
            let c = this.states.get(i);

            if (!c.nextState) {
                c.nextState = this.states.get(i + 1);
            }
        ...
        for (const [id, state] of this.states) {
            console.log(`state ${state.name} -> next ${state.nextState ? state.nextState.name : '<end>'}`);
        }
    ...
    handleBet(player, bet) {
        if (player.state !== this.currentState.name) {
            
        }
        
        state.process(player, bet);
        // return:
        // next player ID, next player OWED, next player ACTIONS
    }
    
    static createFrame(playercurr, playernext) {
