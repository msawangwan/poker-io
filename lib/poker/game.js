...
class Game {
    constructor(id, positions, buttonPosition) {
        ...
        this.gameEvent.once('game-start', (onstart) => {
            ...
            this.dealer.updateRoundStateOnPlayerCompletedAction('start');
    ...
    playerCompletedAction(id, clientRoundName, action, orderIndex, amount) {
        ...
        if (!this.dealer.validateTurnState(id, clientRoundName)) {
            Game.log(
        ...
        const curupdated = this.dealer.updatePlayerStateOnActionCompleted(cur, action, clientRoundName, amount);
        const roundupdated = this.dealer.updateRoundStateOnPlayerCompletedAction(clientRoundName, curupdated, action);

        Game.logRoundState('round after player action validation', clientRoundName, roundupdated.name);
        ...
        clientRoundName = this.dealer.activeRound.name;
        ...
        Game.logPlayerState(curupdated, 'current player');
