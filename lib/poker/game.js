...
class Game {
    constructor(id, players, button) {
        ...
        this.gameEvent.once('posted-small-blind', (id, amount, onpost) => {
            ...
            this.gameEvent.emit('action', this.turnOrder.get(this.currentActionOnPlayer), 'bigblind', this.minbet);
            this.nextToAct();
        ...
        this.gameEvent.once('posted-big-blind', (id, amount, onpost) => {
            ...
            this.gameEvent.emit('action', this.turnOrder.get(this.currentActionOnPlayer), 'anteup', this.minbet);
            this.nextToAct();
    ...
    updatePlayerTurnOrder(index, playerid) {
        if (!playerid in this.players) {
            ...
            if (this.turnOrder.size >= this.players.length) {
                ...
                this.currentActionOnPlayer = 1;
                this.gameEvent.emit('action', this.turnOrder.get(this.currentActionOnPlayer), 'smallblind', this.minbet);
                this.nextToAct();
            }
        }
    };
