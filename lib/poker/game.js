...
class Game {
    constructor(id, positions, buttonPosition) {
        ...
        this.gameEvent.on('player-posted-ante', (playerid, performedAction, bet, onnotify) => {
            ...
            let nextbetmin = bet < this.minbet ? this.minbet : bet;

            if (bets.length) {
                nextbetmin = Math.abs(bets[bets.length - 1] - bet);
            }
            ...
            let callOrCheck = nextbetmin > 0 ? 'call' : 'check';

            if (performedAction === 'raise') {
                allowedActions.push(callOrCheck, 'fold');
                if (numcirc < 3) {
                    allowedActions.push('raise');
                }
            } else {
                allowedActions.push(callOrCheck, 'raise', 'fold');
            }
