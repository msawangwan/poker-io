...
class Game {
    constructor(id, positions, buttonPosition) {
        ...
        this.gameEvent.once('game-start', (onstart) => {
            ...
            // this.dealer.toggleActingPlayerStateByBetOrder(null, 0);
            ...
            // this.dealer.setPlayerStateAllowedActions(sb.id, 'postblind');
    ...
    playerCompletedAction(id, round, action, orderIndex, amount) {
        ...
        if (this.dealer.isPlayerByIdActingPlayer(id)) {
            Game.log(
                `error: client sent id that doesnt match current game state:`,
                `server state current: ${cur.player.id}`,
                `client sent: ${id}`
            );
        }

        if (curround.name !== round) {
            Game.log(
                `error: client sent round that doesnt match current game state:`,
                `server state current: ${curround.name}`,
                `client sent: ${round}`
            );
        }
        ...
        switch (round) {
            case 'blind':
                if (cur.player.isSmallBlind && amount !== this.blindBetSize * 0.5) {
                    amount = this.blindBetSize * 0.5;
                } else if (cur.player.isBigBlind && amount !== this.blindBetSize) {
        ...
        switch (round) {
            case 'blind':
                if (curupdated.player.isBigBlind) {
        ...
        // const next = this.dealer.toggleActingPlayerStateByBetOrder(orderIndex, nextOrderIndex);
