...
const gamestateNotifier = new GameEvent();
const dealerEvent = new GameEvent();
...
function Game(name, id) {
    ...
    this.playerCount = [...this.seating].filter(([k,v]) => v.vacant === false).length;

    this.state = -1;
    
    gamestateNotifier.once('game-started', () => {
        this.state = 1;
    ...
    dealerEvent.on('player-ready', (playerid, onready) => {
        if (!this.readyQueue.has(playerid)) {
            this.readyQueue.add(playerid);
            readyupQueue.push(onready);
            if (this.readyQueue.size === this.playerCount) {
                this.state = 2;
