    ...
    playerCompletedAction(id, round, type, orderIndex, amount) {
        ...
        switch (round) {
            ...
            case 'deal':
                ...
                if (next.id === this.dealer.currentBetAnchor) {
                    Game.log(
                        'reached bet anchor position',
                        `bet order position: ${this.dealer.betAnchorPosition}`
                    );
                } else {
                    // get current bet history or action?
                    // calc how much the next person has to owe?
                    // etc
                    next.allowedActions = ['call', 'bet', 'raise', 'fold']; // TODO: calculate what these actually are
                }
