...
class Game {
    ...
    startTurn(player, state, turnid, matchAmount) {
        ...
        if (turn.round === 'blind') {
            ...
            if (turn.actions.includes('check')) {
        ...
        // if ((turn.actingOrder === 0 || turn.actingOrder === 1) && turn.round === 'blind') {
        //     turn.actions.push('blind');
        // } else {
        //     turn.actions.push('fold');
    ...
    endTurn(pid, turnid, state, actionType, betAmount) {
        ...
        const bet = this.pot.placeBet(pid, state, betType, betAmount);
        ...
        // todo: add to 'this.states.get(state).capCount += 1'
