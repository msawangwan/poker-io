    ...
    constructor(id, positions, buttonPosition) {
        ...
        */
        ...
        this.gameEvent.on('player-posted-ante', (playerid, performedAction, betPlaced, onnotify) => {
            ...
            const dbet = Math.abs(this.currentminbet.size - betPlaced);

            if (dbet === 0) {
                Game.log(`the bet delta is zero`);
            } else {
                Game.log('the bet delta is greater than zero');
            }

            console.log("THIS IS THE BET DELTA X " + dbet);
