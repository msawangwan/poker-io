...
class Game {
    constructor(id, positions, buttonPosition) {
        ...
        this.gameEvent.once('game-start', (onstart) => {
            ...
            this.dealer.toggleActiveRound('start');
        ...
        switch (round) {
            case 'blind':
                ...
                if (!next.isSmallBlind && !next.isBigBlind) {
                    this.dealer.toggleActiveRound(round);
                ...
                nextPlayerAllowedActions = this.dealer.getAllowedActionsForPlayer(next, this.dealer.activeRound.name);

                this.dealer.setAllowedActions(next.id, true, ...nextPlayerAllowedActions);

                break;
            case 'deal':
                completedActionId = this.dealer.completeAction(id, action, round, amount);
