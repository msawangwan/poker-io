class Player {
    ...
    get stacks() {
        return this.allValid.map(p => [p[0], p[1].id, p[1].stackSize]);
    }
    ...
    placeBet(player, round, amount) {
        let bet = amount;

        // all-in
        if (player.stackSize < amount) {
            bet = player.stackSize;
        }

        player.stackSize -= bet;
        player.bets[round] += bet;
