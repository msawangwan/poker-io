            ...
            onjoinhandler(name, this.id, assignedSeat, this.seatingState, () => {
                if (this.vacancyCount(false) > 2) {
                    if (this.state === tablestate.waitingForPlayers) {
                        ...
                        // game.gameEvent.on, ()=> {('phase-change', (current, next) => {
                        //     this.socketconn.in(this.roomId).emit('game-phase-changed', { current: current, next: next });
                        // });
                        ...
                        game.gameEvent.emit('game-start', (playerid) => {
                            const seatIndex = this.players.get(playerid);
                            const turnOrderIndex = (game.button + seatIndex % game.playerCount) % game.playerCount; // TODO: if this is the small blind send a start to it (see game.updatePlayerTurnOrder for deprecated)
                        ...
                        this.button.position += 1 % this.maxseats; // TODO: update this on each turn, get from game
                    }
                }
            });
        });
