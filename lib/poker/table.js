...
class Table {
    constructor(numberOfSeats, socketconn, banker) {
            ...
            onjoinhandler(name, this.id, assignedSeat, this.seatingState, () => {
                if (this.vacancyCount(false) > 2) {
                    if (this.state === tablestate.waitingForPlayers) {
                        ...
                        game.gameEvent.once('predeal', (smallblindId, turnOrder, allowedactions, minbet) => {
        ...
        this.tableEvent.on('game-turn-action', (betActionType, nextBetActionType, betPhase, betAmount, playerid, gameid) => {
            ...
            game.gameEvent.emit(
                ...
                (name, id, amount, potsize, newbalance) => {
