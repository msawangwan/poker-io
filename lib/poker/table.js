...
class Table {
    constructor(numberOfSeats, socketconn, banker) {
            ...
            onjoinhandler(name, this.id, assignedSeat, this.seatingState, () => {
                if (this.vacancyCount(false) > 2) {
                    if (this.state === tablestate.waitingForPlayers) {
                        ...
                        game.gameEvent.on('action', (playerid, actiontype, minbet) => {
                            this.socketconn.to(playerid).emit('action', { type: actiontype, minbet: minbet });
                        });

                        game.gameEvent.on('update-potsize', (potsize) => {
                            this.socketconn.in(this.roomId).emit('update-potsize', { potsize: potsize });
                        });
        ...
        this.tableEvent.on('bet-action', (betActionType, betAmount, playerid, gameid) => {
            ...
            const onbet = (betType, betAmount, potsize, newBalance, seatindex) => {
            ...
            switch (betActionType) {
                case 'smallblind':
                    game.gameEvent.emit('posted-small-blind', playerid, betAmount, (potsize) => {
                        // this.socketconn.in(this.roomId).emit('player-posted-blinds', { playerSeat: seatIndex, betAmount: betAmount, updatedBalance: balance });
                        onbet('smallblind', betAmount, potsize, balance, seatIndex);
                    });
                    break;
                case 'bigblind':
                    game.gameEvent.emit('posted-big-blind', playerid, betAmount, (potsize) => {
                        // this.socketconn.in(this.roomId).emit('player-posted-blinds', { playerSeat: seatIndex, betAmount: betAmount, updatedBalance: balance });
                        onbet('bigblind', betAmount, potsize, balance, seatIndex);
                    });
                    break;
                case 'anteup':
                    game.gameEvent.emit('ante-up-call', playerid, betAmount, (potsize) => {
                        // this.socketconn.in(this.roomId).emit('player-called-ante', { playerSeat: seatindex, betAmount: betAmount, updatedBalance: balance });
                        onbet('anteup', betAmount, potsize, balance, seatIndex);
                    });
                    break;
                case 'raiseante':
                    game.gameEvent.emit('ante-up-raised', playerid, betAmount, (potsize) => {
                        // this.socketconn.in(this.roomId).emit('player-raised-ante', { playerSeat: seatindex, betAmount: betAmount, updatedBalance: balance });
                        onbet('anteraise', betAmount, potsize, balance, seatIndex);
                    });
                    break;
                case 'foldante':
                    game.gameEvent.emit('ante-fold', playerid, betAmount, (potsize) => {
                        // this.socketconn.in(this.roomId).emit('player-folded-on-ante', { playerSeat: seatindex, betAmount: betAmount, updatedBalance: balance });
                        onbet('antefold', betAmount, potsize, balance, seatIndex);
                    });
                    break;
                default:
                    {
