            ...
            onjoinhandler(name, this.id, assignedSeat, this.seatingState, () => {
                if (this.vacancyCount(false) > 3) {
                    if (this.state === tablestate.waitingForPlayers) {
                        ...
                        // game.gameEvent.on('post-blinds', (playeridsb, playeridbb) => {
                        //     socketconn.to(playeridsb).emit('post-blinds', { type: 'small' });
                        //     socketconn.to(playeridbb).emit('post-blinds', { type: 'big' });
                        // });
