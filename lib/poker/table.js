...
class Table {
    constructor(numberOfSeats, socketconn, banker) {
            ...
            onjoinhandler(name, this.id, assignedSeat, this.seatingState, () => {
                if (this.vacancyCount(false) > 2) {
                    if (this.state === tablestate.waitingForPlayers) {
                        ...
                        game.gameEvent.on('notify-next-to-act', (playerid, nextAction, minbet) => {
        ...
        this.tableEvent.on('bet-action', (betActionType, betAmount, playerid, gameid) => {
            ...
            //             onbet('smallblind', betAmount, potsize, balance, seatIndex, this.names.get(playerid), playerid);
            //         });
            //         break;
            //     case 'bigblind':
            //         game.gameEvent.emit('posted-big-blind', playerid, betAmount, (potsize) => {
            //             onbet('bigblind', betAmount, potsize, balance, seatIndex, this.names.get(playerid), playerid);
            //         });
            //         break;
            //     case 'anteup':
            //         game.gameEvent.emit('ante-up-call', playerid, betAmount, (potsize) => {
            //             onbet('anteup', betAmount, potsize, balance, seatIndex, this.names.get(playerid), playerid);
            //         });
            //         break;
            //     case 'checkante':
            //         game.gameEvent.emit('ante-up-check', playerid, betAmount, (potsize) => {
            //             onbet('checkante', betAmount, potsize, balance, seatIndex, this.names.get(playerid), playerid);
            //         });
            //         break;
            //     case 'raiseante':
            //         game.gameEvent.emit('ante-up-raised', playerid, betAmount, (potsize) => {
            //             onbet('anteraise', betAmount, potsize, balance, seatIndex, this.names.get(playerid), playerid);
            //         });
            //         break;
            //     case 'foldante':
            //         game.gameEvent.emit('ante-fold', playerid, betAmount, (potsize) => {
            //             onbet('antefold', betAmount, potsize, balance, seatIndex, this.names.get(playerid), playerid);
            //         });
            //         break;
            //     default:
            //         {
