...
class Table {
    constructor(numberOfSeats, socketconn, banker) {
            ...
            onjoinhandler(name, this.id, assignedSeat, this.seatingState, () => {
                if (this.vacancyCount(false) > 2) {
                    if (this.state === tablestate.waitingForPlayers) {
                        ...
                        game.gameEvent.once('predeal', (smallblindId, order, allowedactions, minbet) => {
                        ...
                        game.gameEvent.once('preflop', (dealtCards, utgid, order, allowedactions, minbet) => {
                            for (const [id, i] of this.players) { // emit as action?
                        ...
                        game.gameEvent.on('notify-next-to-act', (toplayerid, phase, order, allowedactions, minbet) => {
        ...
        this.tableEvent.on('game-turn-action', (betActionType, betAmount, gameid, playerid) => {
            ...
            {
                console.log('===');
                console.log('player completed turn action');
