...
class Table {
    constructor(numberOfSeats, socketconn, banker) {
            ...
            onjoinhandler(name, this.id, assignedSeat, this.seatingState, () => {
                if (this.vacancyCount(false) > this.startThreshold) {
                    if (this.state === tablestate.waitingForPlayers) {
                        ...
                        game.gameEvent.on('deal-flop', (utgById, minBetAmount, flopped) => {
        ...
        this.tableEvent.on('poll-for-updated-table-state', (playerid, gameid) => {
            const game = this.games.get(gameid);

            // todo: send:
            // - whos turn, potsize, and anything else?

            // this.socketconn.to(playerid).emit('update-table-state', {
